...
持久化键含环境、服务端确认 uid、结构版本、规范化动作/查询。最多 12 项/512 KiB，每项 96 KiB、期限24小时；只允许首页、目录、账户、分类、统计和有界流水（最多3页），不存原始证据。落盘采用同步有界操作，世代检查后写入，写失败降级内存；缓存清理仅操作专用键，不碰 pending-ledger-write。
