...
实施未开始。范围见 [README](README.md)，行为约束见 [requirements](requirements.md)，顺序见 [tasks](tasks.md)。一期包含原导入模块化和有边界的文档、相关测试、缓存整理；P0～P5 是一期内部步骤，不是多个产品版本。模块名为建议，职责和真实消费者优先，不预建全部文件。
