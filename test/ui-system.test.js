...
test('账户页以净资产和异常余额为主，行内图标直达改名与停用，点行进详情页', function () {
  ...
  assert.match(template, /catchtap="openRename"[^>]*aria-label="修改 {{item\.name}} 名称"/)
  assert.match(template, /catchtap="archive"[^>]*aria-label="停用 {{item\.name}}"/)
  assert.match(template, /\/assets\/icons\/pencil\.svg/)
  assert.match(template, /\/assets\/icons\/trash\.svg/)
  ...
  assert.match(style, /\.account-row-edit \{[^}]*height:\s*88rpx/)
  assert.match(style, /\.account-row-edit-icon \{[^}]*opacity:\s*\.62/)
  ...
  assert.doesNotMatch(source, /loadAccountLoans|closeAccountDetail/)
...
test('账户详情独立页：余额大卡、流水入口、负债贷款区与操作区', function () {
  ...
  assert.match(markup, /<page-head[^>]*title="{{account \? account\.name : '账户详情'}}"/)
