...
test('跨部署贷款保护的分块、去重与用户范围一致', async () => {
  ...
  const connection = { execute: async (sql, values) => { assert.equal(values[0], 'synthetic-user'); if (sql.includes('catledger_loan_charges')) { assert.match(sql, /uid=\? AND transaction_id IN/); chargeCounts.push(values.length - 1) } else { assert.match(sql, /uid=\? AND active_transaction_id IN/); counts.push(values.length - 1) } return [[]] } }
