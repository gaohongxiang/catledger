...
test('小程序导入调用只使用已登记动作，各导入入口复用独立工作台', function () {
  const page = ['index.js', 'runtime.js', 'upload-flow.js', 'paged.js', 'presentation.js'].map(file => fs.readFileSync(path.join(__dirname, '../miniprogram/pages/import-workbench', file), 'utf8')).join('\n')
  ...
  assert.match(tab, /'\/pages\/transaction-editor\/index'/)
  assert.match(tab, /'\/pages\/import-workbench\/index'/)
...
test('导入工作台沿用按需登录与主题组件', function () {
  ...
  const source = ['index.js', 'runtime.js', 'upload-flow.js', 'paged.js', 'presentation.js'].map(file => fs.readFileSync(path.join(__dirname, '../miniprogram/pages/import-workbench', file), 'utf8')).join('\n')
...
test('导入工作台以多文件 FinanceUpdate 和 ReviewIssue 取代逐行 post/skip', function () {
  ...
  assert.match(source, /maxFiles: uploadFlow\.MAX_FILES/)
  ...
  assert.match(markup, />选择账单<\/text>/)
  assert.match(markup, />确认账户<\/text>/)
  assert.match(markup, />整理账目<\/text>/)
  assert.match(markup, />确认入账<\/text>/)
  assert.match(markup, /选择账单文件/)
  assert.match(markup, /账户归属/)
  assert.match(markup, /整理账目/)
  assert.match(markup, /确认整批入账/)
  assert.match(markup, /点“读取失败”查看具体原因和处理方式/)
  assert.match(markup, /上限 5 份 · 单份 5 MB/)
  assert.doesNotMatch(markup, /添加后可开始/)
  assert.match(markup, />继续添加<\/button>/)
  ...
  assert.match(markup, /每份文件独立处理/)
  assert.match(markup, /解析中…/)
  ...
  assert.match(styles, /\.account-decision-row \{[^}]*min-height: 88rpx;/)
  assert.match(styles, /\.account-compact-name \{[^}]*height: 88rpx;[^}]*line-height: normal;/)
  assert.match(styles, /\.choose-button-secondary\[disabled\] \{[^}]*background-color: var\(--ui-surface-muted\) !important;/)
  assert.match(styles, /\.stage-primary\[disabled\],[\s\S]*?\.final-primary\[disabled\] \{[^}]*background-color: var\(--ui-surface-muted\) !important;/)
  ...
  assert.match(markup, /只继续整理读取成功的账单/)
  ...
  assert.match(markup, /catchtap="removeFile" aria-role="button">删除<\/view>/)
  assert.match(markup, /bindtap="tapFileRow"/)
  assert.doesNotMatch(markup, /file-next-action/)
  ...
  assert.doesNotMatch(styles.match(/\.file-error \{[^}]*\}/)[0], /max-height|overflow: hidden/)
  assert.match(styles, /\.file-row-side \{[^}]*width: 64rpx;[^}]*flex: 0 0 64rpx;/)
  assert.ok(markup.indexOf('class="file-batch"') < markup.indexOf('class="file-picker-actions"'))
  assert.doesNotMatch(markup, /以前|之前/)
  assert.doesNotMatch(markup, /本次所有账单都会回滚/)
  ...
  assert.match(source, /defaultIgnored\s*\?\s*'ignore_future'/)
  ...
  assert.match(markup, /入账前不改动正式账户/)
