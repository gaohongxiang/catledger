...
test('导入工作台以多文件 FinanceUpdate 和 ReviewIssue 取代逐行 post/skip', function () {
  ...
  assert.match(markup, /需要处理的文件已单独列出/)
  ...
  assert.match(markup, /<view wx:if="\{\{!files.length && phase !== 'loading'\}\}" data-ui="empty-picker" class="file-picker-empty">/)
  ...
  assert.match(markup, /支付宝与微信账单，整理到一起/)
  ...
  assert.match(markup, /确认入账后创建，当前不会改动账户余额/)
