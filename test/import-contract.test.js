...
test('导入工作台以多文件 FinanceUpdate 和 ReviewIssue 取代逐行 post/skip', function () {
  ...
  assert.match(markup, /最多 \{\{maxFiles\}\} 份 · 单份不超过 5 MB/)
  assert.doesNotMatch(markup, /添加后可开始/)
  assert.match(markup, />＋ 继续添加账单<\/button>/)
  assert.doesNotMatch(markup, />重新选择(?:文件|账单)?<\/button>/)
  assert.match(markup, /class="file-picker-actions"/)
  assert.equal((markup.match(/class="file-picker-action"/g) || []).length, 1)
  assert.match(markup, /class="primary-button stage-primary parse-button \{\{phase === 'uploading' \? 'parse-button-loading' : ''\}\}"/)
  assert.match(markup, /phase === 'uploading' \? '正在解析' : '开始解析'/)
  assert.match(markup, /各文件独立处理/)
  assert.match(markup, /请稍候，文件进度会自动更新/)
  ...
  assert.match(markup, /<view wx:if="\{\{!files.length && \(phase === 'idle' \|\| phase === 'selected' \|\| phase === 'files_ready'\)\}\}" class="file-empty-state">/)
  ...
  assert.match(styles, /\.choose-button-secondary\[disabled\] \{[^}]*background-color: var\(--theme-surface-muted, #f3f0eb\) !important;/)
  assert.match(styles, /\.stage-primary\[disabled\],[\s\S]*?\.final-primary\[disabled\] \{[^}]*background-color: var\(--theme-surface-muted, #f3f0eb\) !important;/)
  assert.match(markup, /uploadSummary\.queued > 0 \|\| phase === 'uploading'/)
  ...
  assert.match(markup, /phase === 'files_ready' \|\| phase === 'selected' \|\| phase === 'organizing'/)
  ...
  assert.match(markup, /未成功文件不会计入本次整理，可单独重试或移除/)
  ...
  assert.match(markup, /至少一份账单读取成功后才可继续/)
  ...
  assert.match(markup, /已按历史规则自动排除/)
  ...
  assert.match(markup, /item\.suggestedExisting \? '建议账户 · 待确认'/)
