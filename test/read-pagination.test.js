...
test('流水后续页不汇总，版本变化拒绝旧游标；退款口径、身份与组合筛选不变', { skip: !process.env.CATLEDGER_TEST_DB_HOST }, async () => {
  ...
  try {
    ...
    const deletable = first.transactions.find(row => row.transactionId !== original.transactionId)
    assert.ok(deletable, '游标测试只删除没有退款关联的普通支出')
    await api('transactions.delete', { requestId: randomUUID(), transactionId: deletable.transactionId, version: deletable.version })
    observer.reset()
