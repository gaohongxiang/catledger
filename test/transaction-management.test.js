...
test('批量删除、导入记录与整批撤销使用隔离 MySQL 和最小权限', { skip: !process.env.CATLEDGER_TEST_DB_HOST }, async t => {
  ...
  try {
    ...
    await t.test('过期版本、导入混选、不存在、重复ID、空列表和超限都不部分删除', async () => {
      ...
      await assert.rejects(api('transactions.deleteMany', request(rows)), { publicCode: 'NOT_FOUND' })
    ...
    await t.test('导入可查文件和时间，改分类后可撤销，旧预览不得覆盖后来修改；重导仍正常入账', async () => {
      ...
      assert.equal(history.items.length, 1); assert.equal(history.items[0].files[0], '合成原生验证.csv'); assert.equal(history.items[0].status, 'posted')
      ...
      assert.equal(page1.items[0].updateId, reimport.updateId); assert.equal(page2.items[0].updateId, post.updateId); assert.equal(page2.nextCursor, null)
