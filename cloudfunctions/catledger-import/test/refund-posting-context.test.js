...
test('退款批量上下文保持整数累计、已有退款和时间限制，查询按块而不按退款逐笔', async () => {
  ...
  assert.equal(calls.filter(c => !c.sql.includes('FROM catledger_loan_charges')).length, 15)
  assert.equal(calls.filter(c => c.sql.includes('FROM catledger_loan_charges')).length, 1)
