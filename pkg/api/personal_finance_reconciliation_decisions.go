...
func newPersonalFinanceReconciliationCaseResponse(value *reconciliation.CaseSummary) (*personalFinanceReconciliationCaseResponse, error) {
	if value == nil || value.CaseId < 1 || value.Version < 1 || value.MemberCount != 2 || value.CreatedUnixTime < 1 || value.LastEvaluatedUnixTime < 1 || value.UpdatedUnixTime < 1 ||
		...
		(value.CandidateRuleVersion != reconciliation.CANDIDATE_RULE_VERSION_V1 && value.CandidateRuleVersion != reconciliation.CANDIDATE_RULE_VERSION_V2 && value.CandidateRuleVersion != reconciliation.CANDIDATE_RULE_VERSION_V3) ||
		(value.ExplanationVersion != reconciliation.EXPLANATION_VERSION_V1 && value.ExplanationVersion != reconciliation.EXPLANATION_VERSION_V2 && value.ExplanationVersion != reconciliation.EXPLANATION_VERSION_V3) ||
