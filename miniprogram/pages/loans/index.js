...
Page({
  data: { items: [], loading: false, hasLoaded: false, errorMessage: '', nextCursor: null, canPrevious: false },
  ...
  loadLoans(force) {
    const current = pageReadSession.begin(this, ['items','loading','hasLoaded','errorMessage','nextCursor','canPrevious'], ['_load','_cursor','_previous'])
    if (this._load) return this._load
    this.setData({ loading: true, errorMessage: '' })
    this._load = api.callApi('loans.list', { pageSize: 20, cursor: this._cursor || null }, { force: Boolean(force) })
      .then(result => { if (current()) this.setData({ items: result.items.map(present), nextCursor: result.nextCursor, hasLoaded: true, canPrevious: Boolean(this._previous && this._previous.length) }) })
