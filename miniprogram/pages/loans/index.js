...
Page({
  data: { scoped: false, scopeAccount: null, pendingRepaymentCount: 0, items: [], loading: false, hasLoaded: false, errorMessage: '', nextCursor: null, canPrevious: false },
  ...
  loadLoans(force) {
    ...
    this._load = (async () => {
      ...
      if (!current()) return
      const result = await api.callApi('loans.list', this.loanQuery(), { force: Boolean(force) })
      if (!current()) return
      if (this._accountId && result.items.some(item => item.accountId !== this._accountId)) throw new Error('贷款所属账户不一致，请重试')
      this.setData({ pendingRepaymentCount: Number(result.pendingRepaymentCount || 0), items: result.items.map(present), nextCursor: result.nextCursor, hasLoaded: true, canPrevious: Boolean(this._previous && this._previous.length) })
