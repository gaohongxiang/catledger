...
Page({
  data:{ loading:false,saving:false,errorMessage:'',savedMessage:'',hasPending:false,sourceReady:true,sourceLocked:false,loan:null,accounts:[],accountIndex:-1,name:'',principalYuan:'',
  ...
  onLoad(query){this._query=query || {};theme.bindPage(this);this.setData({baselineDate:this._query.baselineDate || model.today(),sourceReady:!this._query.sourceTransactionId || !!this._query.loanId,sourceLocked:!!this._query.sourceTransactionId})},
  ...
  async load(){
    ...
    try{
      ...
      const selected=this.data.accounts[this.data.accountIndex];let accountId=selected && selected.accountId || this._query.accountId
      ...
      this.setData({accounts,accountIndex:accounts.findIndex(a=>a.accountId===accountId)})
  ...
  selectAccount(event){if(!this.data.loan&&!this.data.sourceLocked)this.setData({accountIndex:Number(event.detail.value),confirmed:false})},
  ...
  async save(){
    if(this.data.loading||this.data.saving||!this.data.sourceReady||(this._query.loanId&&!this.data.loan))return
