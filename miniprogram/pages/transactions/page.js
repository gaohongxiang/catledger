...
function createTransactionsPage(accountMode, dependencies) {
const { app, api, themeService, wx } = dependencies
return Object.assign({
  ...
  onLoad: function (query) {
    ...
    if (accountMode) {
      const accountId = String(query && query.accountId || '').trim()
      if (accountId) {
        this._accountEntrySession = readCache.getSession()
        this.setData({ accountFilters: [{ accountId: '', name: '全部账户' }, { accountId, name: '当前账户' }], accountFilterIndex: 1, returnAccountId: accountId })
      }
      else this.setData({ errorMessage: '账户参数无效' })
    }
  },
  ...
  onShow: function () {
    ...
    const incoming = accountMode ? null : app.globalData.transactionsImportFilter
    if (!accountMode) app.globalData.transactionsImportFilter = null
    if (incoming && incoming.session === readCache.getSession() && app.hasLoginApproval()) {
      this.setData({ importFilter: incoming, accountFilterIndex: 0, returnAccountId: '', categoryFilterIndex: 0, sourceFilterIndex: 0, search: '', appliedSearch: '', searchOpen: false, selectedDate: '', selectedDateLabel: '', transactions: [], nextCursor: null, hasLoaded: false })
    }
    this.resetSelection()
    this.setData({ selectionMode: false, deleteRetryCount: 0 })
    this._batchRequest = null
    themeService.bindPage(this)
    if (!accountMode && this.getTabBar()) {
      this.getTabBar().setData({ selected: 1 })
    }
    const loggedIn = app.hasLoginApproval()
    this.setData({ loggedIn: loggedIn })
    if (loggedIn && (!accountMode || this.data.returnAccountId)) {
      this.prepareAndLoad()
      return
    }
    this.setData({
  ...
  prepareAndLoad: function (options) {
    ...
    if (accountMode && !this.data.returnAccountId) return Promise.resolve()
  ...
  changeAccountFilter: function (event) {
    ...
    this.setData({ accountFilterIndex: index, returnAccountId: accountMode ? this.data.returnAccountId : '', hasLoaded: false, transactions: [] })
    return this.loadTransactions(false)
  },

  returnToAccountDetail: function () {
    if (!accountMode || !this.data.returnAccountId || this.data.importFilter ||
        this.data.deleting || this.data.selectingAll || this._returningToAccount || !app.hasLoginApproval()) return
    this._returningToAccount = true
    wx.navigateBack({ delta: 1, fail: () => { this._returningToAccount = false; wx.showToast({ title: '返回失败，请重试', icon: 'none' }) } })
  },
  ...
  onWechatLoginSuccess: function () {
    this.setData({ loggedIn: true, returnAccountId: '' })
    this.prepareAndLoad()
  }
}, batchDelete)
}

module.exports = { createTransactionsPage }
