...
function build(loan, view, preview, date) {
  ...
  return {
    ...
    progressNote: view && view.tracking ? '' : history ? '历史已还 ' + history + ' 期' + (summary ? ' · 接入后已还 ' + Number(summary.paidPeriods || 0) + ' 期' : '') : '',
    ...
    dueLabel: pending ? '正在读取还款计划' : complete ? '本笔已完成' : noPlan ? '还款计划待补充' : next <= date ? view && view.tracking ? '缺少账单，待补充' : '待确认还款' : '近期应还',
    ...
    dueNote: complete ? '已记录全部期次' : noPlan ? '补充后查看每期应还' : '点期次补充账单或确认进度',
    ...
    remaining: cost.amount(noPlan ? null : remaining), remainingPrincipal: cost.amount(noPlan ? loan.remainingPrincipalMinor : summary && summary.unpaidPrincipalMinor),
