...
Page({
  ...
  data: { ...require('./charge-actions').initial,chargeRefundAccounts:[],sourceTransactionId: '', sourceContext: null, loan: null, loading: false, saving: false, errorMessage: '', fieldError: '', savedMessage: '', formOpen: false, hasPending: false,
    ...
    repaymentRows: [], detail: null, detailLoading: false, detailError: '', periodRows: [], scheduleMore: false, scheduleHistorical: false, guideOpen: false,
  ...
  openPayment(event) {
    if (!pageReadSession.isCurrent(this) || !this.data.periodOpen || this.data.periodLoading || this.data.saving) return
    const id = event.currentTarget.dataset.id
    if (!this.data.periodLegacy.some(payment => payment.paymentId === id)) return
    this.closeInstallment()
    wx.navigateTo({ url: '/pages/loan-payment/index?paymentId=' + encodeURIComponent(id) })
  },
