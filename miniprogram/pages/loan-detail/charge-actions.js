...
const methods={
 async loadCharges(event){
  ...
  try{
   ...
   this.setData({chargeRows:result.items.map(rowView),chargeNext:result.nextCursor,chargeIssues:result.issues.map(i=>({...i,amountText:money.formatMinor(i.amountMinor)})),
    chargeSummary:{recorded:money.formatMinor(result.recordedMinor),unverified:money.formatMinor(result.unverifiedMinor),cutoff:result.cutoff,
 ...
 async openChargeEdit(event){
  const current=session.capture(this),id=event.currentTarget.dataset.id,issue=this.data.chargeIssues.find(i=>i.eventId===id)
  let item=this.data.chargeRows.find(i=>i.chargeId===id)
  if(issue){try{const view=await api.callApi('loans.chargePlan',{loanId:this._loanId,periodNumber:issue.periodNumber},{force:true});if(!current())return;item=view.items.find(i=>i.component===issue.component&&i.chargeKey==='period:'+issue.periodNumber+':'+issue.component)}catch(error){if(current())this.setData({chargeError:error.message});return}}
  if(!item){this.setData({chargeError:'请先确认这期收费覆盖，再核对账单'});return}
  this.setData({chargeEdit:{...rowView(item),eventId:issue&&issue.eventId||'',updateId:issue&&issue.updateId||'',amountYuan:money.minorToYuan(issue?issue.amountMinor:item.amountMinor),refundDate:this._chargeView.cutoff,refundAccountIndex:-1,operation:'adjust'},chargeImpact:null})
 },
 chargeEditInput(event){const field=event.currentTarget.dataset.field;if(['amountYuan','refundDate','refundAccountIndex'].includes(field))this.setData({['chargeEdit.'+field]:field==='refundAccountIndex'?Number(event.detail.value):event.detail.value,chargeImpact:null})},
 async previewChargeChange(event){
  const d=this.data.chargeEdit,current=session.capture(this);if(!d||this.data.saving)return
  try{const operation=event.currentTarget.dataset.operation,data={loanId:this._loanId,chargeId:d.chargeId,operation,...(d.eventId?{eventId:d.eventId}:{}),...(['adjust','distinct','refund'].includes(operation)?{amountMinor:money.yuanToMinor(d.amountYuan)}:{})}
   ...
   const result=await api.callApi('loans.chargeImpact',data,{force:true});if(current()){this._chargeChange=data;this.setData({chargeImpact:{...result,deltaText:money.formatMinor(result.deltaMinor),additionalText:money.formatMinor(result.nextPostingMinor||'0')}})}
  }catch(error){if(current())this.setData({chargeError:error.message})}
 },
 async confirmChargeChange(){const impact=this.data.chargeImpact;if(!impact||!impact.canChange||this.data.saving)return;const current=session.capture(this);if(!await confirm({title:'确认这笔费用变化',content:'账本费用净变化 '+impact.deltaText+' 元。'+(this._chargeChange.operation==='distinct'?'另有待导入收费 '+impact.additionalText+' 元。':'')+'本金与银行卡付款不会自动改变。',confirmText:'确认处理'})||!current())return
  const saved=await this.installmentWrite('loans.changeCharge',{...this._chargeChange,confirmed:true,previewToken:impact.previewToken});if(saved&&current())this.setData({chargeEdit:null,chargeImpact:null})
 },
